...
ipcMain.on('ques_data:ready', (e, isReady, numQues) => {
  ...
  if (isReady) {
    ...
    fs.readFile(json_file, (error, raw_data) => {
      ...
      mainWindow.webContents.send('ques_data:add', ques_data_array.slice(0, numQues));
    })
  }
...
ipcMain.on('test_results:ready', (e, grade, results) => {
...
ipcMain.on('results_page:ready', (e, isReady) => {
  ...
  if (isReady) {
    ...
    //Send the Data to the Screen
    mainWindow.webContents.send('results_data:add', results_grade, results_data, ques_data_array);
  }
  //Not Ready
  else {
